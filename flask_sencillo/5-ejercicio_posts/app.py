...
# Para tomar peticiones que nos vengan de tipo POST-GET
...
## Podemos eliminar un objeto a traves de una consulta sobre nuestra tabla Posts
@app.route("/listar/posts")
def listar_posts():
    todos_posts =  Posts.query.order_by(Posts.id).all() # Seleccionamos todos los posts y ordenamos por id
    # Pasamos a la vista el listado de objetos con todos los posts para que los renderice como quiera
    return render_template("listar_posts.html", listado_posts=todos_posts)


# Ruta para eliminar un post a partir de su identificador único
@app.route("/eliminar/post/<int:post_id>")
def eliminar_post(post_id):
    first_post = Posts.query.filter_by(id=post_id).first()
    db.session.delete(first_post)
    db.session.commit()
    # Volvemos a la pagina con todos los posts
    todos_posts =  Posts.query.order_by(Posts.id).all() # Seleccionamos todos los posts y ordenamos por id
    return render_template("listar_posts.html", listado_posts=todos_posts)


# Ruta para eliminar un post a partir de su identificador único
@app.route("/editar/post/<int:post_id>")
def editar_post(post_id):
    post_selecionado = Posts.query.filter_by(id=post_id).first()
    return render_template("editar_posts.html", post=post_selecionado)


@app.route("/actualizar/post/", methods=['POST'])
def actualizar_post():
    post_id = request.form['id_post']
    post_titulo = request.form['titulo_post']

    selected_post = Posts.query.filter_by(id=post_id).first()
    selected_post.titulo = post_titulo
    db.session.commit()
